/* eslint-disable import/prefer-default-export */

export const SET_RUNTIME_VARIABLE = 'SET_RUNTIME_VARIABLE';

// Home

export const FETCH_POSTS_PENDING = 'FETCH_POSTS_PENDING';
export const FETCH_POSTS_SUCCESS = 'FETCH_POSTS_SUCCESS';
export const FETCH_POSTS_FAILURE = 'FETCH_POSTS_FAILURE';
export const UPDATE_BASIC_LIST_PENDING = 'UPDATE_BASIC_LIST_PENDING';
export const UPDATE_BASIC_LIST_SUCCESS = 'UPDATE_BASIC_LIST_SUCCESS';
export const UPDATE_BASIC_LIST_FAILURE = 'UPDATE_BASIC_LIST_FAILURE';

// Assignments

export const FETCH_ASSIGNMENTS_PENDING = 'FETCH_ASSIGNMENTS_PENDING';
export const FETCH_ASSIGNMENTS_SUCCESS = 'FETCH_ASSIGNMENTS_SUCCESS';
export const FETCH_ASSIGNMENTS_FAILURE = 'FETCH_ASSIGNMENTS_FAILURE';

// Grades

export const FETCH_GRADES_PENDING = 'FETCH_GRADES_PENDING';
export const FETCH_GRADES_SUCCESS = 'FETCH_GRADES_SUCCESS';
export const FETCH_GRADES_FAILURE = 'FETCH_GRADES_FAILURE';

// Messages

export const FETCH_MESSAGES_PENDING = 'FETCH_MESSAGES_PENDING';
export const FETCH_MESSAGES_SUCCESS = 'FETCH_MESSAGES_SUCCESS';
export const FETCH_MESSAGES_FAILURE = 'FETCH_MESSAGES_FAILURE';
export const FETCH_MESSAGE_PENDING = 'FETCH_MESSAGE_PENDING';
export const FETCH_MESSAGE_SUCCESS = 'FETCH_MESSAGE_SUCCESS';
export const FETCH_MESSAGE_FAILURE = 'FETCH_MESSAGE_FAILURE';

// Notes

export const FETCH_NOTES_PENDING = 'FETCH_NOTES_PENDING';
export const FETCH_NOTES_SUCCESS = 'FETCH_NOTES_SUCCESS';
export const FETCH_NOTES_FAILURE = 'FETCH_NOTES_FAILURE';
export const FETCH_NOTE_PENDING = 'FETCH_NOTE_PENDING';
export const FETCH_NOTE_SUCCESS = 'FETCH_NOTE_SUCCESS';
export const FETCH_NOTE_FAILURE = 'FETCH_NOTE_FAILURE';

// Classes

export const FETCH_CLASSES_PENDING = 'FETCH_CLASSES_PENDING';
export const FETCH_CLASSES_SUCCESS = 'FETCH_CLASSES_SUCCESS';
export const FETCH_CLASSES_FAILURE = 'FETCH_CLASSES_FAILURE';
export const FETCH_CLASS_PENDING = 'FETCH_CLASS_PENDING';
export const FETCH_CLASS_SUCCESS = 'FETCH_CLASS_SUCCESS';
export const FETCH_CLASS_FAILURE = 'FETCH_CLASS_FAILURE';
